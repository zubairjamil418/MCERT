...
export class McertsFormData {
  ...
  // thirdary Device Information
  @Prop()
  thirdaryDeviceTransmitter?: string;

  @Prop()
  thirdaryDeviceSensor?: string;

  @Prop()
  thirdaryDeviceCompliance?: string;

  @Prop({ type: [FileUpload] })
  thirdaryDeviceImages?: FileUpload[];

  @Prop({ type: [String] })
  thirdaryDeviceCaptions?: string[];

  @Prop()
  thirdaryDeviceDescription?: string;
...
@Schema({ timestamps: true, collection: 'third_forms_collection' })
export class thirdForm extends Document {
...
export const thirdFormSchema = SchemaFactory.createForClass(thirdForm);

// Add database indexes for better query performance
thirdFormSchema.index({ userId: 1 });
thirdFormSchema.index({ status: 1 });
thirdFormSchema.index({ createdAt: -1 });
thirdFormSchema.index({ userId: 1, status: 1 });
thirdFormSchema.index({ dataSize: 1 });
