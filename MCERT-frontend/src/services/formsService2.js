...
export const formsService = {
  ...
  getAllForms: async (
    ...
    const url = `${SECOND_FORMS_API.GET_ALL}?page=${page}&limit=${limit}&sortOrder=${sortOrder}`;
  ...
  getFormById: async (id, token = null) => {
    const headers = token ? getAuthHeaders(token) : DEFAULT_HEADERS;
    return makeApiCall(SECOND_FORMS_API.GET_BY_ID(id), {
      method: HTTP_METHODS.GET,
      headers,
    });
  },
  ...
  createForm: async (formData, token = null) => {
    const headers = token ? getAuthHeaders(token) : DEFAULT_HEADERS;
    return makeApiCall(SECOND_FORMS_API.CREATE, {
  ...
  updateForm: async (id, formData, token = null) => {
    const headers = token ? getAuthHeaders(token) : DEFAULT_HEADERS;
    return makeApiCall(SECOND_FORMS_API.UPDATE(id), {
  ...
  deleteForm: async (id, token = null) => {
    const headers = token ? getAuthHeaders(token) : DEFAULT_HEADERS;
    return makeApiCall(SECOND_FORMS_API.DELETE(id), {
  ...
  submitForm: async (formData, token = null) => {
    const headers = token ? getAuthHeaders(token) : DEFAULT_HEADERS;
    return makeApiCall(SECOND_FORMS_API.SUBMIT, {
      method: HTTP_METHODS.POST,
      headers,
      body: JSON.stringify(formData),
    });
  ...
  generateReport: async (formData, token = null) => {
    const headers = token ? getAuthHeaders(token) : DEFAULT_HEADERS;
    return makeApiCall(SECOND_FORMS_API.GENERATE_REPORT, {
      method: HTTP_METHODS.POST,
      headers,
      body: JSON.stringify(formData),
    });
